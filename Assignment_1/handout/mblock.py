...
def pad_block(hex_msg,block_size):
    ...
    # Splitting the last block of our message in block of 2
    msg_blocks = hex_msg
    print (msg_blocks)
    # One full block is 8 block of 2, so we are searching how much we need to add
    ...
    if(msg_len != 16):
        ...
        while( i < n):
            ...
            # We have got a problem here, I would like to add 030303 to the hex_msg
            # but hex_msg is in hex and padstring is a string so I have an 
            # "Can't concatenate error" but if I transform 030303 in hex, I can add it 
...
def enc_msg_cbc(iv,hex_msg_blocks):
    ...
    for i in range(0,len(hex_msg_blocks)):
        ...
        # Transform the bytes in string, not sure of that by the way
        if(i == 0):
