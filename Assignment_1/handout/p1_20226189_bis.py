# We have to run this command on a termianl first : 
...
# Also made some research on google to see how to do things, never really coded in python before :(
...
# So to do that, we will need to split the cipher in 
# lenght of block size
    ...
    for block in reversed(range(0,len(ciphertext_block))):
        for i in range (0, size_block):
            ...
            for j in range(0, 256):
                ...
                # the value that match the padding in the oracle

                if j != i+1 or (len(correct_value) > 0 and int(correct_value[-1],16) == j):
                    print ("-------------NEW ITER J-------------")
                    print ("iter j = ", j)
                    ...
                    # We are using the IV but if we got multiple 
                    previous_block = ciphertext_block[block-1]
                    ...
                    # We are tryng to change block[-1]
                    ...
                    current_cipherblock = ciphertext_block[block]
                    print ("current_cipherblock : ", current_cipherblock)
...
# We are putting them in a string with ''.join(result)
    ...
    print ("Decrypted value in HEX is :", hex_value)

    hex_value= hex_value[16:]
    print ("Decrypted value in HEX without the IV is :", hex_value)
# We convert our result in hex in ascii

    decrypted_value = size_block_split(hex_value, text_size)
    print("decrypted_value = ", decrypted_value)
    final_result = []
    for i in range (0, len(decrypted_value)):
        final_result = bytes.fromhex(decrypted_value[i]).decode('utf-8')
        print ("Decrypted value in ASCII is : ", final_result)
